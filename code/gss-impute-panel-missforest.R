# Preliminaries 
rm(list = ls())
sink("log/gss-impute-panel-missforest.log")

#### Description ###############################################################

# Impute core variables for the GSS panel, using the missForest package

#### Set up ####################################################################

# Set your working directory here (should be main folder of GitHub repository):
#setwd("") 
...
num.of.cores = 4
registerDoParallel(cores = num.of.cores)

#### Load data from Stata (as a csv file) ######################################
...
# Create flags for missing data and a data file to store them
imputevars_m <- data 
imputevars_m[ , paste0( "m_",names(imputevars_m)[-1])] <- 
  lapply(imputevars_m[-1], function(x) as.numeric(is.na(x)))
imputevars_m <- imputevars_m[ , grepl( "m_" , names( imputevars_m ) ) ]
imputevars_m <- cbind(data$year, data$samptype,  data$id, data$panelid, 
                      data$panelwave, imputevars_m)
for (j in c("year", "samptype", "id", "panelid", "panelwave")) {
  names(imputevars_m)[names(imputevars_m) == paste0("data$",j)] <- j
}

print(summary(data))
print(summary(imputevars_m))

# Convert columns with categorical data from numeric to factor
for (k in c("year", "samptype", "panelwave", "degree", "uscitzn", "region", 
            "dwelown", "xnorcsiz", "marital", "relig", "class", "spdeg", 
            "madeg", "padeg", "wrkstat", "sex", "born","parborn", "family16", 
            "reg16", "incom16", "intsex", "intethn", "coop", "comprend", 
            "spaneng", "mode", "feeused", "egp10_11", "spegp10_11", 
            "paegp10_11", "maegp10_11", "racehisp5", "evercitzn")) {
...
# Set to 6 imputations and 5 iterations, using 500 trees. Could be modified.

num.impute = 6
for (i in 1:num.impute) {

  # Random forest imputation
  imputed <- missForest::missForest(data[, -which(names(data) == "panelid")],
                                    maxiter = 5, ntree = 500, verbose = TRUE,
  ...
  imputed <- data.frame(lapply(imputed, 
                               function(x) as.numeric(as.character(x))))
  
  # Create an indicator variable for imputation dataset
  imputed$flag <- rep(i,nrow(imputed))

  # Save imputed dataset as an RData file
  done.time <- gsub("[:, ]", "-", trimws(Sys.time()))
  saveRDS(imputed, file = sprintf("tmp/imputed-%s.RData", done.time))
  
  # Print summary
  print(i)
  print(summary(imputed))

  # Assign new name to imputed object with _i appended to name
  assign(paste("imputed_",i, sep=""), imputed)
  
}

#### Gather imputations into a single dataset and write out as a Stata file ####

# Append all imputed datasets
imputed_appended <- as.data.frame(rbind(imputed_1, imputed_2, imputed_3, 
                                        imputed_4, imputed_5, imputed_6))

# Merge flags for missing
imputed_appended <- merge(imputed_appended, imputevars_m, by=c("year", "id", 
                                                               "samptype"))
...
print(summary(imputed_appended))

# Save as a Stata dta file
foreign::write.dta(imputed_appended,"data/gss-panel-vars-imputed-rf.dta")

# Print time and close sink
print(Sys.time())
